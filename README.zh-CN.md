...
成功加载的文章会自动写入持久化浏览历史。历史和收藏在不同文库之间使用统一列表，但每条记录仍保留来源文库和规范化后的服务器地址；界面只显示当前服务器的记录。数据位于系统应用数据目录（Linux 默认为 `~/.local/share/kiwix-cli/reading.json`），可用 `KIWIX_CLI_DATA_DIR` 覆盖其父目录。
